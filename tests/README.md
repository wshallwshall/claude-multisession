...
**Python 3 standard library only.** Nothing here needs an external tool.

That changed with the split. This suite used to require `pandoc` on `PATH`, because
`test_word_copies_track_the_markdown.py` rebuilt every published Word copy and compared it against
the committed one. The Word copies belong to the standards, and both went to
[secure-development-standards](https://github.com/wshallwshall/secure-development-standards) --
along with the rule-identifier, heading-citation and selector-parity checks, which pinned documents
this repository no longer holds.
