...
| Script | Does | Doc |
|---|---|---|
| `scripts/cron/watch-ci-red.ps1` | Polls for pull requests your repository labelled red and starts one session per red to attribute it. Zero model calls, so a quiet repository is free. Exits non-zero rather than reporting a zero it could not establish, and on a claim whose seat has died. `-DryRun`, `-Json` | [Watching for a red](CI-RED-WATCH.md) |
