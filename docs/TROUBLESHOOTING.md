...
|---|---|---|
| The doctor says `STALE` | The installed copy's SHA-256 differs from this checkout's source. The code that runs is not the code you are reading. Reported `RED`. | Re-run that control's installer, from a plain terminal. Vendoring leaves two copies on disk; install and audit from the same one. |
...
| Presence lists nobody | Either nobody is live, or the roster could not be read. Both print an empty list. | Read the exit code rather than the rows. `0` is a complete roster, including one that lists nobody. `2` is a roster that could not be completed. |
...
| Every commit and every push is refused | `_ccxconfig.py` is missing from the git hooks directory, or differs from source. The checkers cannot import their substrate, so they refuse everything. | Re-run `install-git-hooks.ps1`. The doctor reports this as `RED` and says the checker refuses every commit rather than the unclaimed one. |
...
| Sequences are configured and nothing enforces them | No `pre-commit` hook invokes `seq_check.py`. The doctor prints `OFF` on that row and does not fail the run, so exit 0 is reached with the hole open. | Wire `scripts/hooks/seq_check.py` into whatever `pre-commit` you already run. That `OFF` row is the only place this is visible. |

### The green run and the real hole

The sequence gate is the one row where a clean verdict and a missing control sit together. No
shipped installer writes `pre-commit`, because two tools cannot both own that file. Failing the run
on it would redden every clean install.

So the row is recorded as not required. It appears as `OFF (opt-in)` in the verdict, and the exit
code stays 0. The run is green.
...
stops in three cases. There is no `ccx.config.json` at or above the path; that file will not load;
the path is not inside a git repository.

### `-SkipAttacks` cannot prove enforcement

`-SkipAttacks` fires nothing. Every attack is then reported `??`, so the run cannot exit 0. It
exits 2, or 1 if a receipt check also found something broken or absent.
